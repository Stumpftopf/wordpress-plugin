...
class Measurement
{
	public $value;
	public $unit;
	public $trafficlight = array("green" => "#C8F526", 
								"yellow" => "#FFDB58",
								 "red" => "#FFB093"); 
...
class Wind_speed extends Measurement
{
	...
	private function GetQuality()
	{
		...
		if ($val <= 18)
			...
			$this->color=$this->trafficlight['yellow'];
		else
...
class Wind_Direction extends Measurement
{
	...
	public $name_long;
	public $svg_arrow;
	public $color; //suitability at best takeoff
	public $best_takeoff; 
    	...
    	foreach ($takeoffs as $takeoff)
    		{
    			...
    			if ($this->value >= $takeoff->winddirection_levels[1][0]
    			 && $this->value <= $takeoff->winddirection_levels[1][1]); //return takeoff if value is within boundaries
    				return $takeoff;
    		}
